...
test('Get Value: Unsigned Intel', () => {
  const can = new Can();
  expect(can.extractValFromPayload([20, 255, 255, 2, 4, 13, 16, 255], 40, 9, 'Intel', false)).toBe(13);
  expect(can.extractValFromPayload([54, 20, 45, 2], 10, 10, 'Intel', false)).toBe(837);
});

test('Get Value: Signed Intel', () => {
  const can = new Can();
  expect(can.extractValFromPayload([20, 255, 255, 2, 4, 13, 16, 255], 20, 27, 'Intel', true)).toBe(13647919);
  expect(can.extractValFromPayload([20, 255, 255, 2, 4, 13, 16, 255], 13, 32, 'Intel', true)).toBe(1746933759);
  expect(can.extractValFromPayload([20, 255, 255, 2, 4, 13, 16, 255], 56, 3, 'Intel', true)).toBe(-1);
  expect(can.extractValFromPayload([54, 20, 45, 2], 10, 10, 'Intel', true)).toBe(-187);
});
